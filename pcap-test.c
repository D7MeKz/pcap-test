...
int main(int argc, char* argv[]) {
		...
		printf("smac ");
    		...
    		printf("Data: ");
                if (header->caplen > sizeof(*eth_hdr) + (ip_hdr->ip_hl * 4) + sizeof(*tcp_hdr)) {
                    const u_char* data = packet + sizeof(*eth_hdr) + (ip_hdr->ip_hl * 4) + sizeof(*tcp_hdr);
                    for (int i = 0; i < 10 && i < header->caplen - sizeof(*eth_hdr) - (ip_hdr->ip_hl * 4) - sizeof(*tcp_hdr); i++) {
                        printf("%02X ", data[i]);
                    }
                } else {
                    printf("None");
